...
{
    bool_t doit = FALSE;
    bool_t sf=GET_SF(cc),of=GET_OF(cc),zf=GET_ZF(cc);
	switch(cond)
	{
		case C_YES:
			doit=TRUE;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    byte_t ireg=0;
    if(icode==I_RRMOVQ||icode==I_IRMOVQ||icode==I_RMMOVQ||icode==I_MRMOVQ||icode==I_PUSHQ||icode==I_POPQ||icode==I_ALU)
    {
		if(!get_byte_val(sim->m,next_pc, &ireg))
		{
	        err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
	        return STAT_ADR;
		}
    ...
    {
    	if(!get_long_val(sim->m,next_pc, &inum))
    	{
	        err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
	        return STAT_ADR;
		}
	...
	long_t regA,regB,iVal=0,nowrsp=0;
	cond_t icond;
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
      	regA=GET_REGA(ireg),regB=GET_REGB(ireg);
		...
		icond=(cond_t)ifun;
      	if(cond_doit(sim->cc,icond)==TRUE)
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
      	...
      	if(!set_long_val(sim->m,inum+get_reg_val(sim->r,regB),get_reg_val(sim->r,regA)))
      	{
	    	err_print("PC = 0x%lx, Invalid data address 0x%.16lx", sim->pc, inum+get_reg_val(sim->r,regB));
	    	return STAT_ADR;
		}
      	break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
		...
		iVal=0;
		if(!get_long_val(sim->m,get_reg_val(sim->r,regB)+inum,&iVal))
      	{
	    	err_print("PC = 0x%lx, Invalid data address 0x%.16lx", sim->pc, get_reg_val(sim->r,regB)+inum);
	    	return STAT_ADR;
		}
      	set_reg_val(sim->r,regA,iVal);
      	break;
      case I_ALU: /* 6:x regA:regB */
      	...
      	iVal=compute_alu(ifun,get_reg_val(sim->r,regA),get_reg_val(sim->r,regB));
      	sim->cc=compute_cc(ifun,get_reg_val(sim->r,regA),get_reg_val(sim->r,regB),iVal);
      	set_reg_val(sim->r,regB,iVal);
      	break;
      case I_JMP: /* 7:x imm */
      	icond=(cond_t)ifun;
      	if(inum<0||inum>=sim->m->len)
      	{
	        err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
	        return STAT_ADR;
		}
      ...
      case I_CALL: /* 8:x imm */
      	...
      	if(cond_doit(sim->cc,icond)==TRUE)
      	{
      		nowrsp=get_reg_val(sim->r,REG_RSP)-8;
      		set_reg_val(sim->r,REG_RSP,nowrsp);
      		if(!set_long_val(sim->m,nowrsp,next_pc))
      		{
      			err_print("PC = 0x%lx, Invalid stack address 0x%.16lx", sim->pc,nowrsp);
	      		return STAT_ADR;
			}
      		next_pc=inum;
      	}
      	break;
      case I_RET: /* 9:0 */
      	nowrsp=get_reg_val(sim->r,REG_RSP);
      	if(!get_long_val(sim->m,nowrsp,&next_pc))
      	{
      		err_print("PC = 0x%lx, Invalid stack address 0x%.16lx", sim->pc,nowrsp);
      		set_reg_val(sim->r,REG_RSP,nowrsp+8);
	        return STAT_ADR;
		}
		if(next_pc<0||next_pc>=sim->m->len)
      	{
      		err_print("PC = 0x%lx, Invalid stack address 0x%.16lx", sim->pc,next_pc);
     	 	set_reg_val(sim->r,REG_RSP,nowrsp+8);
	    	return STAT_ADR;
		}
      	set_reg_val(sim->r,REG_RSP,nowrsp+8);
      ...
      case I_PUSHQ: /* A:0 regA:F */
      	nowrsp=get_reg_val(sim->r,REG_RSP)-8;
      	if(!set_long_val(sim->m,nowrsp,get_reg_val(sim->r,HIGH(ireg))))
  		{
  			err_print("PC = 0x%lx, Invalid stack address 0x%.16lx", sim->pc,nowrsp);
      		set_reg_val(sim->r,REG_RSP,nowrsp);
      		return STAT_ADR;
      ...
      case I_POPQ: /* B:0 regA:F */
      	nowrsp=get_reg_val(sim->r,REG_RSP);
      	iVal=0;
		if(!get_long_val(sim->m,nowrsp,&iVal))
  		{
  			err_print("PC = 0x%lx, Invalid stack address 0x%.16lx", sim->pc,nowrsp);
      		set_reg_val(sim->r,REG_RSP,nowrsp+8);
      		return STAT_ADR;
      	...
      	set_reg_val(sim->r,REG_RSP,nowrsp+8);
      	set_reg_val(sim->r,HIGH(ireg),iVal);
    	break;
      default:
    	err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc, codefun);
    	return STAT_INS;
    }
//    long_t checkrsp=get_reg_val(sim->r,REG_RSP);
//    if(checkrsp<0||checkrsp>=sim->m->len)
//    {
//    	err_print("PC = 0x%lx, Invalid data address %.2x", sim->pc, codefun);
//    	return STAT_ADR;
//	}
    sim->pc=next_pc;
    return STAT_AOK;
