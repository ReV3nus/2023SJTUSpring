...
word f_pc = [
	D_icode == IJXX && E_icode == IOPQ && E_ifun == ALUAND && D_ifun == CLE && E_srcA == E_srcB && E_valA > 0 : D_valP;
...
bool E_bubble =
	...
	(D_icode == IJXX && E_icode == IOPQ && E_ifun == ALUAND && D_ifun == CLE && E_srcA == E_srcB && E_valA > 0) ||
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd) ||
	# Conditions for a load/use hazard
	E_icode in { IMRMOVQ, IPOPQ } &&
