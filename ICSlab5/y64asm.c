...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    int slen=0;
	while(IS_LETTER((*ptr)+slen)||IS_NUMBER((*ptr)+slen))slen++;
...
parse_t parse_mem(char **ptr, long long *value, regid_t *regid)
{
	...
	if(IS_DIGIT(*ptr))
	{
		if(parse_digit(ptr,&multmp)==PARSE_ERR)
		{
			err_print("Invalid MEM");
			return PARSE_ERR;
		}
...
//						err_print("Invalid directive");
				...
				case D_ALIGN:
					line->y64bin.codes[0]=0;
					stap=parse_digit(&pos,&value);
					if(stap==PARSE_DIGIT)vmaddr+=value-((vmaddr+(value-1))%value+1);
					else stap=-1;
					line->y64bin.addr=vmaddr;
					break;
				default:
...
//    		puts("###");
			while(siz<pos)op[siz]=0,siz++;
//    		puts("###");
			for(int i=0;i<tmp->y64bin.bytes;i++)
			{
				if(pos<siz)op[pos]=tmp->y64bin.codes[i];
				else siz++,op[pos]=(tmp->y64bin.codes[i]);
				pos++;
			}
		}
        tmp = tmp->next;
    }
//    for(int i=0;i<siz;i++)printf("%2x ",op[i]);
//    printf("\n");
    /* binary write y64 code to output file (NOTE: see fwrite()) */
    fwrite(op,sizeof(byte_t),sizeof(byte_t)*siz,out);
    return 0;
}
